...
async function main(): Promise<void> {
	...
	const DATA = await parseDirectoryWithResults("dfs", parameters);
	...
	await fs.writeFile('./dfs_AVERAGE_LENGTH.png', buffer, 'base64');
	...
	configuration = createConfiguration({
		canvasHeight: height,
		canvasWidth: width,
		title: 'DFS - AVERAGE VISITED STATES',
        type: 'logarithmic',
		datasets: _.map(parameters, (param, idx) => {
			return {
				label: param.toUpperCase(),
	...
	await fs.writeFile('./dfs_AVERAGE_VISITED_STATES.png', buffer, 'base64');

	//#3. AVERAGE PROCESSED STATES
	configuration = createConfiguration({
		canvasHeight: height,
		canvasWidth: width,
		title: 'DFS - AVERAGE PROCESSED STATES',
        type: 'logarithmic',
		datasets: _.map(parameters, (param, idx) => {
			return {
				label: param.toUpperCase(),
	...
	buffer = await chartJSNodeCanvas.renderToBuffer(configuration);
	await fs.writeFile('./dfs_AVERAGE_PROCESSED_STATES.png', buffer, 'base64');

	//#4. AVERAGE DEPTH ACHIEVED
	configuration = createConfiguration({
		canvasHeight: height,
		canvasWidth: width,
		title: 'DFS - AVERAGE MAXIMUM DEPTH ACHIEVED',
        type: 'linear',
		datasets: _.map(parameters, (param, idx) => {
			return {
				label: param.toUpperCase(),
	...
	buffer = await chartJSNodeCanvas.renderToBuffer(configuration);
	await fs.writeFile('./dfs_AVERAGE_DEPTH_ACHIEVED.png', buffer, 'base64');

	//#5. AVERAGE DEPTH ACHIEVED
	configuration = createConfiguration({
		canvasHeight: height,
		canvasWidth: width,
		title: 'DFS - AVERAGE SOLVING DURATION [ms]',
        type: 'logarithmic',
		datasets: _.map(parameters, (param, idx) => {
			return {
				label: param.toUpperCase(),
	...
	buffer = await chartJSNodeCanvas.renderToBuffer(configuration);
	await fs.writeFile('./dfs_AVERAGE_SOLVING_DURATION.png', buffer, 'base64');
}
main();