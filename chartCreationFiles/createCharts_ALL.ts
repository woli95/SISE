...
const parseDirectoryWithResults = async(parameters:Array<string>) => {
	let FS = import('fs');
	...
	const files = _.filter((await FS).readdirSync('../413ukladow'), (file) => {return file.includes("stats")});
	for(let i = 0; i < parameters.length; i++) {
...
async function main(): Promise<void> {
	...
	const parameters = ['astr', 'bfs', 'dfs'];
	const chartJSNodeCanvas = new ChartJSNodeCanvas({ width, height, chartCallback });
	const DATA = await parseDirectoryWithResults(parameters);
	...
	await fs.writeFile('./all_AVERAGE_LENGTH.png', buffer, 'base64');
	...
	configuration = createConfiguration({
		canvasHeight: height,
		canvasWidth: width,
		title: 'OVERALL - AVERAGE VISITED STATES',
        type: 'logarithmic',
		datasets: _.map(parameters, (param, idx) => {
			return {
				label: param.toUpperCase(),
	...
	await fs.writeFile('./all_AVERAGE_VISITED_STATES.png', buffer, 'base64');

	//#3. AVERAGE PROCESSED STATES
	configuration = createConfiguration({
		canvasHeight: height,
		canvasWidth: width,
		title: 'OVERALL - AVERAGE PROCESSED STATES',
        type: 'logarithmic',
		datasets: _.map(parameters, (param, idx) => {
			return {
				label: param.toUpperCase(),
	...
	buffer = await chartJSNodeCanvas.renderToBuffer(configuration);
	await fs.writeFile('./all_AVERAGE_PROCESSED_STATES.png', buffer, 'base64');

	//#4. AVERAGE DEPTH ACHIEVED
	configuration = createConfiguration({
		canvasHeight: height,
		canvasWidth: width,
		title: 'OVERALL - AVERAGE MAXIMUM DEPTH ACHIEVED',
        type: 'linear',
		datasets: _.map(parameters, (param, idx) => {
			return {
				label: param.toUpperCase(),
	...
	buffer = await chartJSNodeCanvas.renderToBuffer(configuration);
	await fs.writeFile('./all_AVERAGE_DEPTH_ACHIEVED.png', buffer, 'base64');

	//#5. AVERAGE SOLVING DURATION
	configuration = createConfiguration({
		canvasHeight: height,
		canvasWidth: width,
		title: 'OVERALL - AVERAGE SOLVING DURATION [ms]',
        type: 'logarithmic',
		datasets: _.map(parameters, (param, idx) => {
			return {
				label: param.toUpperCase(),
	...
	buffer = await chartJSNodeCanvas.renderToBuffer(configuration);
	await fs.writeFile('./all_AVERAGE_SOLVING_DURATION.png', buffer, 'base64');
}
main();